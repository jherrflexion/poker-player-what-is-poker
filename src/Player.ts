export class Player {
  ...
  public betRequest(gameState: any, betCallback: (bet: number) => void): void {
    try {
      ...
      if (!holeCards.length) {
        console.log('No hole cards found, folding');
        betCallback(0);
        return;
      }
      ...
      console.log(`Betting ${betAmount} with hand strength ${handStrength}`);
  ...
  public showdown(gameState: any): void {
    // Log the showdown state to learn from it
    console.log('Showdown state:', JSON.stringify(gameState, null, 2));
    
    // TODO: Implement learning from showdowns
  }
